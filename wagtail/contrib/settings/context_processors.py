...
class SiteSettingModuleProxy(dict):
    """
    Get a site-specific setting instance using proxy['modelname']
    """
    ...
    def __str__(self):
        return "SiteSettingModuleProxy({0})".format(self.app_label)
...
class SiteSettingProxy(dict):
    """
    Get a SiteSettingModuleProxy for an app using proxy['app_label']
    """
    ...
    def __missing__(self, app_label):
        self[app_label] = value = SiteSettingModuleProxy(
            self.request_or_site, app_label
        )
        return value
...
def site_settings(request):
    ...
    def _inner(request):
        ...
        if site is None:
            ...
            return SiteSettingProxy(request)

    return {"site_settings": SimpleLazyObject(lambda: _inner(request))}


class GenericSettingProxy(dict):
    """
    Get a GenericSettingModuleProxy for an app using proxy['app_label']
    """
    ...
    def __missing__(self, app_label):
        self[app_label] = value = GenericSettingModuleProxy(self.request, app_label)
        return value


class GenericSettingModuleProxy(dict):
    """
    Get a generic setting instance using proxy['modelname']
    """
    ...
    def __str__(self):
        return "GenericSettingModuleProxy({0})".format(self.app_label)
...
def generic_settings(request):
    # delay query until settings values are needed
    def _inner(request):
        return GenericSettingProxy(request)

    return {"generic_settings": SimpleLazyObject(lambda: _inner(request))}
