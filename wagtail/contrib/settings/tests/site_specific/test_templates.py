...
class TestContextProcessor(TemplateTestCase):
    def test_accessing_setting(self):
        ...
        self.assertEqual(
            self.render(
                request, "{{ site_settings.tests.site_specific.TestSiteSetting.title }}"
            ),
    ...
    def test_model_case_insensitive(self):
        ...
        self.assertEqual(
            self.render(
                request, "{{ site_settings.tests.site_specific.testsetting.title }}"
            ),
            self.default_site_settings.title,
        )
        self.assertEqual(
    ...
    def test_models_cached(self):
        ...
        request = self.get_request()
        get_title = "{{ site_settings.tests.site_specific.testsetting.title }}"
...
class TestTemplateTag(TemplateTestCase):
    def test_no_context_processor(self):
        ...
        """
        template = Template(
            "{{ site_settings.tests.site_specific.TestSiteSetting.title }}"
        )
        context = Context()
        self.assertEqual(template.render(context), "")

    def test_get_settings_request_context(self):
        """Check that the {% get_site_settings %} tag works"""
        request = self.get_request(site=self.other_site)
        ...
        template = Template(
            ...
            "{% get_site_settings %}"
            "{{ site_settings.tests.site_specific.testsetting.title}}"
        )
    ...
    def test_get_settings_request_context_use_default(self):
        """
        Check that the {% get_site_settings use_default_site=True %} option
    ...
    def test_get_settings_use_default(self):
        """
        Check that the {% get_site_settings use_default_site=True %} option works
        """
        context = Context()
    ...
    def test_get_settings_no_request_no_default(self):
        """
        Check that the {% get_site_settings %} throws an error if it can not find a
        site to work with
        """
        context = Context()
    ...
    def test_get_settings_variable_assignment_request_context(self):
        ...
        {% get_site_settings as wagtail_settings %} works.
        """
        request = self.get_request(site=self.other_site)
        context = Context({"request": request})
        template = Template(
            ...
            "{{ wagtail_settings.tests.site_specific.testsetting.title}}"
        )
    ...
    def test_get_settings_variable_assigment_use_default(self):
        """
        Check that assigning the setting to a context variable with
        {% get_site_settings use_default_site=True as wagtail_settings %} works.
        """
        context = Context()
        template = Template(
            "{% load wagtailsettings_tags %}"
            "{% get_site_settings use_default_site=True as wagtail_settings %}"
            "{{ wagtail_settings.tests.site_specific.testsetting.title }}"
        )
...
class TestSiteSettingsJinja(TemplateTestCase):
    ...
    def test_model_case_insensitive(self):
        """Model names should be case insensitive"""
        self.assertEqual(
            self.render('{{ site_settings("tests.site_specific.testsetting").title }}'),
            self.default_site_settings.title,
        )
        self.assertEqual(
            self.render('{{ site_settings("tests.site_specific.TESTSETTING").title }}'),
    ...
    def test_models_cached(self):
        ...
        for i in range(1, 4):
            with self.assertNumQueries(1):
                ...
                self.assertEqual(
                    template.render(context), self.default_site_settings.title * i
                )

    def test_settings_use_default_site_override(self):
        """
        Check that {{ site_settings(use_default_site=True) }} overrides a site in
        the context.
        """
        request = self.get_request(site=self.other_site)
        context = {"request": request}
        ...
        # This should use the default site, ignoring the site in the request
        template = '{{ site_settings("tests.site_specific.testsetting", use_default_site=True).title }}'

        self.assertEqual(
            self.render(template, context), self.default_site_settings.title
        )

    def test_settings_use_default_site(self):
        """
        Check that the {{ site_settings(use_default_site=True) }} option works with
    ...
    def test_settings_no_request_no_use_default(self):
        """
        Check that {{ site_settings }} throws an error if it can not find a
        site to work with
        """
        context = {}
        ...
        # Without a request in the context, and without use_default_site, this
        # should bail with an error
        template = '{{ site_settings("tests.site_specific.testsetting").title}}'
        with self.assertRaises(RuntimeError):
            self.render(template, context, request_context=False)
